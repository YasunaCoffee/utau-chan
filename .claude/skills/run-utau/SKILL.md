---
name: run-utau
description: うたうちゃん(このリポジトリ)を起動して歌わせる。「うたを鳴らして」「うたわせて」「実行して」「WAVにして」「ブラウザで開いて」「run-utau」などと言われたとき、または譜面(.uta)を書いて音を確認したいときに使う。CLI(node bin/sing.js)でWAV生成、ブラウザ(index.html)で再生の両方に対応。
---
...
- **CLI** — `node bin/sing.js 譜面.uta` で WAV を書き出す(Node.jsだけ)
- **ブラウザ** — `index.html` を開いて再生(Windows 97風UI)
...
```bash
node bin/sing.js songs/kirakira.uta     # → songs/kirakira.wav ができる
```
...
```bash
for f in songs/*.uta; do node bin/sing.js "$f"; done
```

### CLIの他の使い方

```bash
node bin/sing.js songs/kirakira.uta -o /tmp/out.wav   # 出力先を指定
echo "@tempo 120
ど4:ら ど4:ら そ4:そ:8" | node bin/sing.js -           # 標準入力から(→ song.wav)
node bin/sing.js --help                                # 書式ヘルプ
```
...
node bin/singteto.js songs/kirakira.uta --bank "<音源フォルダ>"
UTAU_BANK="<音源フォルダ>" node bin/singteto.js songs/kirakira.uta
node bin/singteto.js songs/kirakira.uta --no-vib --dry  # ビブラート/残響オフ
```
...
- **ブラウザからも使える**: `index.html` の「TETO.EXE — 実録音でうたう」で音源フォルダを指定すると、GUIだけで実録音再生・WAV保存ができる(読み込みはブラウザ内で完結)。
- **⚠️ 音源ファイルは同梱・コミット禁止**。重音テト音源(小山乃舞世)は再配布不可のため、各自ダウンロードしてリポジトリ外に置く。`singteto.js` はコードのみ(音源も生成WAVも含まない)。

## 新しい譜面を書いて鳴らす(要点)

`高さ:歌詞[:長さ][調教記号]` を空白区切りで並べる。詳細は `README.md` と `node bin/sing.js --help`。
...
書いたら `node bin/sing.js 新譜面.uta` で即確認。パースエラーは「譜面がよめませんでした」と行が示されるので、その行を直す。
...
```bash
node bin/sing.js songs/kirakira.uta -o /tmp/smoke.wav && \
...
```bash
python3 -m http.server 8000   # → http://localhost:8000/band.html
```

サンプルチップ(⚡テクノ風/🎧ポップス風/🌙うた入りバラード)か `songs/*.band` の内容を貼って▶。演奏中にテキストを直して「✎ 差し替え」(またはCtrl+Enter)すると次の小節から鳴り変わる。「🔗 共有リンク」を押すとURLの `#b=` にバンドテキストが埋め込まれるので、そのまま送って共有できる。パーサだけならNodeでも `require('./src/synth.js').parse(text)` で確認可能。詳しい書式は `README.md` の「バンドちゃん」セクションへ。
