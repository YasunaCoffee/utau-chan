/*
 * teto-web.js — うたうちゃん × 重音テト(ブラウザ版)
 *
 * singteto.js(Node CLIのミニUTAUリサンプラー)をブラウザへ移植したもの。
 * フォルダ選択で読み込んだUTAU音源(録音WAV + oto.ini)を切り貼りして、
 * うたテキスト譜面を実録音の声で歌わせる。合成アルゴリズムはCLI版と同一で、
 * I/Oだけ fs → File/ArrayBuffer に置き換えてある。
 *
 * 一度読み込んだ音源は IndexedDB にキャッシュするので、次に index.html を
 * 開いたときは自動で復元される(音源そのものは同梱しない/再配布禁止のため)。
 *
 *   使い方(index.html 側):
 *     await TetoBank.loadFromFileList(input.files)   // フォルダ選択の File 群
 *     await TetoBank.restoreFromIDB()                // 起動時の自動復元
 *     const {master} = TetoBank.render(song, {vib, reverb})
 */
(function (global) {
  'use strict';
  ...
  /* ---- 状態(読み込んだ音源) ---- */
  // BANK = {name, oto:{map,vcv}, wavs:Map<相対パス, ArrayBuffer>}
  let BANK = null;
  const wavCache = new Map();   // filename -> {data:Float32Array}
  const f0Cache = new Map();

  function resetCaches() { wavCache.clear(); f0Cache.clear(); }
  ...
  /* ---- WAV(16bit PCM mono)を ArrayBuffer から読む ---- */
  function decodeWav(ab) {
    const dv = new DataView(ab);
    const ascii = (p) => String.fromCharCode(dv.getUint8(p), dv.getUint8(p + 1), dv.getUint8(p + 2), dv.getUint8(p + 3));
    let p = 12, fmt = null, dataOff = 0, dataLen = 0;
    while (p + 8 <= ab.byteLength) {
      const id = ascii(p), sz = dv.getUint32(p + 4, true);
  ...
  function readWav(file) {
    ...
    const ab = BANK.wavs.get(file) || BANK.wavs.get(file.split(/[\\/]/).pop()); // 相対パス→basenameで代替
    if (!ab) { const res = { data: new Float32Array(0) }; wavCache.set(file, res); return res; }
    const res = { data: decodeWav(ab) };
  ...
  function buildNote(rec, midi, durSamp, seed, phraseStart, useVib) {
  ...
  function render(song, opts) {
    ...
    const jobs = [];
    let prevV = '-'; const missing = new Set();
    for (const n of notes) {
      ...
      const built = buildNote(rec, n.midi, Math.round(n.len * st * SR), jobs.length + 1, prevV === '-', useVib);
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      const XFMAX = ms(60);
      const fiLen = Math.max(ms(6), Math.min(ovOut, XFMAX));
      let nxt = null;
      ...
      if (nxt) {
        ...
        const nFi = Math.max(ms(6), Math.min(nxt.built.ovOut, XFMAX));
      ...
      for (let k = 0; k < outLen && start + k < master.length; k++) {
        let s = buf[k];
        if (k < fiLen) s *= Math.sin(0.5 * Math.PI * k / fiLen);
        const dEnd = outLen - 1 - k;
        if (dEnd < foLen) s *= Math.sin(0.5 * Math.PI * dEnd / foLen);
    ...
    return { master, placed, missing: [...missing], vcv: BANK.oto.vcv };
  }

  /* ================= 読み込み(フォルダ選択) ================= */

  // File[] (webkitdirectory) から oto.ini を探し、参照WAVを取り込んで BANK を構築
  async function loadFromFileList(fileList) {
    const files = Array.from(fileList);
    const rel = f => f.webkitRelativePath || f.name;
    const otoFile = files.find(f => rel(f).split(/[\\/]/).pop().toLowerCase() === 'oto.ini');
    if (!otoFile) throw new Error('選んだフォルダに oto.ini が見つかりません(音源フォルダ直下を選んでね)');

    const otoRel = rel(otoFile);
    const dir = otoRel.slice(0, otoRel.lastIndexOf('/') + 1); // oto.ini のあるフォルダ
    const otoText = new TextDecoder('shift_jis').decode(await otoFile.arrayBuffer());
    const oto = parseOto(otoText);

    // oto が参照するファイルだけ取り込む(容量節約)。相対パス と basename の両方で引けるように。
    const wanted = new Set();
    for (const rec of oto.map.values()) { wanted.add(rec.file); wanted.add(rec.file.split(/[\\/]/).pop()); }
    const wavs = new Map();
    for (const f of files) {
      const r = rel(f);
      if (!r.startsWith(dir)) continue;
      const key = r.slice(dir.length);        // oto.ini からの相対
      const base = key.split(/[\\/]/).pop();
      if (!/\.wav$/i.test(base)) continue;
      if (wanted.has(key) || wanted.has(base)) {
        const ab = await f.arrayBuffer();
        wavs.set(key, ab);
        if (!wavs.has(base)) wavs.set(base, ab);
      }
    }
    if (!wavs.size) throw new Error('oto.ini に対応するWAVが見つかりませんでした');

    const name = (dir.replace(/\/$/, '').split('/').pop()) || otoRel.split('/')[0] || '音源';
    BANK = { name, oto, wavs };
    resetCaches();
    await saveToIDB({ name, otoText, wavs });
    return info();
  }

  function info() {
    if (!BANK) return null;
    return { name: BANK.name, vcv: BANK.oto.vcv, samples: BANK.wavs.size, aliases: BANK.oto.map.size };
  }
  function isReady() { return !!BANK; }

  /* ================= IndexedDB キャッシュ ================= */
  const DB_NAME = 'utau-teto-bank', STORE = 'bank', KEY = 'current';
  function openDB() {
  ...
  // 起動時: キャッシュがあれば BANK を復元。無ければ null を返す。
  async function restoreFromIDB() {
    try {
      ...
      db.close();
      if (!rec || !rec.otoText || !rec.wavs) return null;
      const wavs = rec.wavs instanceof Map ? rec.wavs : new Map(Object.entries(rec.wavs));
      BANK = { name: rec.name || '音源', oto: parseOto(rec.otoText), wavs };
      resetCaches();
      return info();
    } catch (e) { console.warn('[teto] IndexedDB復元に失敗:', e); return null; }
  }
  async function clearIDB() {
    BANK = null; resetCaches();
    try {
      const db = await openDB();
  ...
  global.TetoBank = { loadFromFileList, restoreFromIDB, clearIDB, render, isReady, info, SR };
})(window);
